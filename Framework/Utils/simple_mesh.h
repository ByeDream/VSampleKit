...
namespace Framework
{
	...
	*/
	void BuildTorusMesh(BuildMeshMode const eMode, IAllocator* allocator, SimpleMesh *destMesh, float outerRadius, float innerRadius, uint16_t outerQuads, uint16_t innerQuads, float outerRepeats, float innerRepeats);
	...
	* @param innerRepeats The number of times that UV coordinates repeat around the inner radius
	*/
	void BuildTorusMesh(BuildMeshMode const eMode, Allocators* allocators, const char *name, SimpleMesh *destMesh, float outerRadius, float innerRadius, uint16_t outerQuads, uint16_t innerQuads, float outerRepeats, float innerRepeats);

	/** @brief Legacy version of BuildTorusMesh.
	*/
	static inline void BuildTorusMesh(IAllocator* allocator, SimpleMesh *destMesh, float outerRadius, float innerRadius, uint16_t outerQuads, uint16_t innerQuads, float outerRepeats, float innerRepeats)
	{
	...
	/** @brief Legacy version of BuildTorusMesh.
	*/
	static inline void BuildTorusMesh(Allocators* allocators, const char *name, SimpleMesh *destMesh, float outerRadius, float innerRadius, uint16_t outerQuads, uint16_t innerQuads, float outerRepeats, float innerRepeats)
	{
	...
	*/
	void BuildSphereMesh(IAllocator* allocator, SimpleMesh *destMesh, float radius, long xdiv, long ydiv, float tx = 0.0f, float ty = 0.0f, float tz = 0.0f);
	...
	* @param tz The Z coordinate of the sphere's center
	*/
	void BuildSphereMesh(Allocators* allocators, const char *name, SimpleMesh *destMesh, float radius, long xdiv, long ydiv, float tx = 0.0f, float ty = 0.0f, float tz = 0.0f);
	...
	*/
	void BuildQuadMesh(IAllocator* allocator, SimpleMesh *destMesh, float size);

	/** @brief Creates a SimpleMesh in the shape of a quadrilateral.
	* @param destMesh The mesh to receive the quadrilateral shape
	* @param size Half the length of a quadrilateral edge
	*/
	void BuildQuadMesh(Allocators* allocators, const char *name, SimpleMesh *destMesh, float size);
	...
	*/
	void BuildCubeMesh(IAllocator* allocator, SimpleMesh *destMesh, float side);

	/** @brief Creates a SimpleMesh in the shape of a cube.
	* @param destMesh The mesh to receive the cube shape
	* @param side The length of one side of the cube
	*/
	void BuildCubeMesh(Allocators* allocators, const char *name, SimpleMesh *destMesh, float side);
	...
	*/
	void LoadSimpleMesh(IAllocator* allocator, SimpleMesh* simpleMesh, const char* filename);
	...
	* @param filename The path to the SimpleMesh file to load
	*/
	void LoadSimpleMesh(Allocators* allocators, SimpleMesh* simpleMesh, const char* filename);
