...
void Framework::BuildTorusMesh(BuildMeshMode const eMode, Allocators* allocators, const char *name, Framework::SimpleMesh *destMesh, float outerRadius, float innerRadius, uint16_t outerQuads, uint16_t innerQuads, float outerRepeats, float innerRepeats)
{
...
void Framework::BuildTorusMesh(BuildMeshMode const eMode, IAllocator* allocator, Framework::SimpleMesh *destMesh, float outerRadius, float innerRadius, uint16_t outerQuads, uint16_t innerQuads, float outerRepeats, float innerRepeats)
{
...
void Framework::BuildSphereMesh(Allocators* allocators, const char *name, SimpleMesh *destMesh, float radius, long xdiv, long ydiv, float tx, float ty, float tz)
{
	...
	for (long i = 0; i < xdiv; ++i)
	{
		...
		for (long j = 1; j < ydiv; ++j)
		{
			...
			outV[k + j].m_color = Vector4(1, 0, 0, 1);
		}
	}
	...
	for (long j = 1; j < ydiv; ++j)
	{
		...
		outV[kk + j].m_color = Vector4(1, 0, 0, 1);
	...
	for (long i = 0; i < xdiv; i++)
	{
		...
		outV[k1 - 1].m_color = Vector4(1, 0, 0, 1);
		...
		outV[k1 + ydiv - 1].m_color = Vector4(1, 0, 0, 1);
...
void Framework::BuildSphereMesh(IAllocator* allocator, SimpleMesh *destMesh, float radius, long xdiv, long ydiv, float tx, float ty, float tz)
{
	Allocators allocators(*allocator, *allocator);
	return BuildSphereMesh(&allocators, 0, destMesh, radius, xdiv, ydiv, tx, ty, tz);
}

void Framework::BuildQuadMesh(Allocators* allocators, const char *name, Framework::SimpleMesh *destMesh, float size)
{
...
void Framework::BuildQuadMesh(IAllocator* allocator, Framework::SimpleMesh *destMesh, float size)
{
	Allocators allocators(*allocator, *allocator);
	return BuildQuadMesh(&allocators, 0, destMesh, size);
}

void Framework::BuildCubeMesh(Allocators* allocators, const char *name, SimpleMesh *destMesh, float side)
{
...
void Framework::BuildCubeMesh(IAllocator* allocator, SimpleMesh *destMesh, float side)
{
	Allocators allocators(*allocator, *allocator);
...
void Framework::LoadSimpleMesh(Allocators* allocators, SimpleMesh* simpleMesh, const char* filename)
{
	FILE* file;
...
void Framework::LoadSimpleMesh(IAllocator* allocator, SimpleMesh* simpleMesh, const char* filename)
{
	Allocators allocators(*allocator, *allocator);
...
void Framework::Mesh::compress(const sce::Gnm::DataFormat *dataFormat, const Framework::SimpleMesh *source, IAllocator* allocator)
{
...
void Framework::tessellate(Mesh *destination, const Mesh *source, IAllocator* allocator)
{
